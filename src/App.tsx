...
const TOTAL_GAME_ROUNDS = 3; // Total de rondas
...
export default function App() {
  ...
  const enterFullScreen = () => {
    try {
      ...
      if (requestFullScreen) {
          requestFullScreen.call(docEl).catch((err: any) => console.log("Error fullscreen", err));
      }
    } catch (e) {
  ...
  const initManualTeams = () => {
    ...
    for (let i = 0; i < numTeams; i++) {
      initialTeams.push({ id: i, name: `Equipo ${i + 1}`, members: [], score: 0, nextDescriberIndex: 0 });
    }
  ...
  const getGuesser = () => {
    ...
    if (team.members.length === 2) {
      ...
      return team.members[guesserIndex]?.name || "Compañero";
    }
      ...
      <div className="min-h-screen w-full bg-green-900 flex flex-col items-center font-sans p-4 relative overflow-x-hidden">
              ...
              <div className="flex-1 flex flex-col justify-center">
                {papelitoStep === 0 ? (
                  <div className="transform -rotate-1">
                      ...
                      <input autoFocus type="text" placeholder="Tu Nombre" className={`w-full text-2xl p-4 font-serif text-blue-900 placeholder-blue-300 outline-none ${NOTEBOOK_PAPER_CLASS}`} style={NOTEBOOK_LINES_STYLE} value={tempPlayerName} onChange={(e) => setTempPlayerName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleNextInputStep()} />
                  </div>
                ) : (
                  <div className="transform rotate-1">
                      ...
                      <div className={`w-full h-32 p-0 ${NOTEBOOK_PAPER_CLASS} shadow-lg`}>
                        <textarea autoFocus placeholder="..." className="w-full h-full bg-transparent resize-none text-2xl font-serif text-gray-800 outline-none px-4" style={NOTEBOOK_LINES_STYLE} value={tempPapelitos[papelitoStep - 1]} onChange={(e) => updateTempPapelito(e.target.value)} onKeyDown={(e) => { if(e.key === 'Enter') { e.preventDefault(); handleNextInputStep(); } }} />
                      </div>
                  </div>
                )}
              </div>
        ...
        <div className="mt-8 w-full max-w-md bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/20 relative z-10">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {players.length === 0 && <span className="text-green-300 italic text-sm">Nadie registrado aún...</span>}
          </div>
          {players.length >= 2 && (
        ...
        <div className="min-h-screen w-full bg-green-900 p-6 flex flex-col justify-center items-center text-white font-sans relative overflow-x-hidden">
            ...
            <div className="w-full max-w-sm space-y-6 z-10">
                ...
                <button onClick={initManualTeams} className="w-full bg-blue-600 hover:bg-blue-700 p-6 rounded-2xl shadow-xl flex flex-col items-center gap-2 transform transition hover:scale-105">
                    ...
                    <span className="text-blue-200 text-sm">Arrastra a tus compañeros</span>
                </button>
    ...
    return (
        <div className="min-h-screen w-full bg-green-900 p-4 flex flex-col items-center font-sans relative overflow-x-hidden overflow-y-auto">
            ...
            <div className="w-full max-w-lg z-10">
                <h3 className="text-xl text-green-200 font-bold mb-4 text-center">{selectedPlayerId ? "Ahora toca el equipo destino 👇" : "Toca un jugador para moverlo 👆"}</h3>
        ...
        <div className="w-full max-w-md space-y-6 mb-8 z-10">
          {teams.map((team) => (
            <div key={team.id} className="bg-white/95 text-gray-800 p-4 rounded-sm shadow-xl transform rotate-1 hover:rotate-0 transition-transform relative">
              ...
              <div className="flex flex-wrap gap-2">
                {team.members.map(m => ( <span key={m.id} className="text-gray-600 font-handwriting text-lg">{m.name} •</span> ))}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-green-900 p-8 rounded-xl shadow-2xl max-w-md w-full border-4 border-wood-800 relative overflow-hidden">
          ...
          <div className="space-y-6 mb-10 relative z-10">
            <div className="bg-black/30 p-4 rounded-lg backdrop-blur-sm border border-white/10">
              <p className="text-xs text-green-300 mb-1 uppercase">Pasa al pizarrón</p>
  ...
  if (phase === 'round_finished') {
    ...
    return (
      <div className="min-h-screen w-full bg-purple-900 flex flex-col items-center p-6 text-white text-center font-sans overflow-x-hidden">
        ...
        <h1 className="text-4xl font-black mb-2 text-white z-10">¡FIN DE LA RONDA {currentGameRound}!</h1>
        ...
        <p className="text-xs text-purple-300 mt-4 z-10">Los papelitos volverán a la bolsa.</p>
      </div>
    );
  }
      ...
      <div className="min-h-screen w-full bg-gray-900 flex flex-col items-center p-6 text-white text-center font-sans overflow-x-hidden">
        ...
        {isTie ? (
             <div className="bg-white text-gray-900 p-8 rounded-lg shadow-xl w-full max-w-md mb-8 transform rotate-1">
                <h2 className="text-3xl font-serif font-bold mb-2">¡Empate Técnico!</h2>
             </div>
        ) : (
